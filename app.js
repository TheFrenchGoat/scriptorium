...
ipcMain.handle('grammar:start-server', async () => {
  ...
  currentProcess.on('error', () => {
    if (languageToolProcess === currentProcess) {
      languageToolProcess = null;
      languageToolReadyPort = null;
      if (win) win.webContents.send('grammar:server-stopped');
    }
  });
  currentProcess.on('exit', () => {
    if (languageToolProcess === currentProcess) {
      languageToolProcess = null;
      languageToolReadyPort = null;
      // Le renderer garde son propre état "serveur prêt" en mémoire (pour ne
      // pas re-vérifier à chaque frappe) : sans ce signal, un crash du
      // process Java après un démarrage réussi (OOM, tué manuellement...)
      // laissait le renderer croire le serveur toujours en ligne, jusqu'à ce
      // qu'une vérification échoue avec une erreur confuse ("pas démarré")
      // alors qu'il avait bel et bien démarré.
      if (win) win.webContents.send('grammar:server-stopped');
    }
  });
